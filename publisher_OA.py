...
def file_opener():
    with open("dirslist_OA.txt", "r") as file:
...
def parser():
    ...
    while len(done_list) != max_index:
        ...
        try:
            new_conn_location = "http://ftp.ebi.ac.uk/pub/databases/pmc/suppl/OA/%s" % current_target.replace("\n", "")
                ...
                if len(soup.findAll('a')) <= 1:
                    ...
                    for files in soup.findAll('a'):
                        ...
                        if "PMC" in files['href']:
                            new_path = "/pub/databases/pmc/suppl/OA/%s" % current_target.replace("\n","") + files['href']
